...
# ------------------------------------------------------------
# 2. Data preprocessing: ML-100K / ML-1M
# ------------------------------------------------------------
def process_ml100k_data(orig_file, out_dir, forget_ratio=0.01):
...
def process_ml1m_data(orig_file, out_dir, forget_ratio=0.01):
    df = pd.read_csv(orig_file, sep="\t", names=["user", "item", "rating", "timestamp"])
...
def train_and_unlearn(args):
    ...
    dataset_name = "ml-100k" if args.use_ml100k else "ml-1m"
    ...
    if not os.path.exists(f"{processed_dir}/train_normal.csv"):
        if args.use_ml100k:
            process_ml100k_data(raw_inter_path, processed_dir, forget_ratio=0.01)
        else:
            process_ml1m_data(raw_inter_path, processed_dir, forget_ratio=0.01)

    # Generate RecBole CSV
    ...
    combined.to_csv(f"{processed_dir}/{dataset_name}_train.csv", index=False)
    vd_raw.to_csv(f"{processed_dir}/{dataset_name}_valid.csv", index=False)
    tt_raw.to_csv(f"{processed_dir}/{dataset_name}_test.csv", index=False)
    ...
    save_full = os.path.join(saved_dir, f"{dataset_name}_{args.model.lower()}_full.pth")
    ...
    save_un = os.path.join(saved_dir, f"{dataset_name}_{args.model.lower()}_unlearned.pth")
    ...
    for k in config["topk"]:
        ...
        orig_recall, unlearn_recall = (
            original_forget_results[f"recall@{k}"],
            unlearn_forget_results[f"recall@{k}"],
        )
    ...
    results_file = os.path.join(results_dir, f"{dataset_name}_{args.model}_scif_results.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--if_lr", type=float, default=2e-5)
    parser.add_argument("--use_ml100k", action="store_true")
