...
class SCIF_Unlearner:
    def __init__(self, model, data_gen, device,
                 if_epoch=1000, if_lr=1e-4, reg=0.01):
        self.model    = model
...
def train_and_unlearn(args):
    ...
    if not os.path.exists(f"{dp}/train_normal.csv"):
        if args.use_ml100k:
            process_ml100k_data("scif/ml-100k.inter", dp, forget_ratio=0.01)
        else:
            process_ml1m_data("scif/ml-1m.inter", dp, forget_ratio=0.01)
...
if __name__=="__main__":
    ...
    p.add_argument("--if_lr",    type=float, default=2e-5)
