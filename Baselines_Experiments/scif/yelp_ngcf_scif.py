...
def calculate_zrf(unlearned_model, incompetent_teacher, forget_samples, batch_size, device):
    ...
    class ForgetDataset(Dataset):
        def __init__(self, samples):
            self.users = torch.LongTensor([u for u, _ in samples])
...
# ------------------------------------------------------------
# 2. Data preprocessing: Yelp
# ------------------------------------------------------------
def process_yelp_data(orig_file, out_dir, forget_ratio=0.01):
    # Raw Yelp file has no header; columns: user, item, rating, timestamp, plus extras we drop.
    df = pd.read_csv(orig_file, sep="\t", header=None)
    if df.shape[1] < 4:
        raise ValueError(f"Unexpected Yelp data format; expected at least 4 columns, got {df.shape[1]}")
    df = df.iloc[:, :4]
...
# ------------------------------------------------------------
# 4. SCIF Unlearner (CPU)
# ------------------------------------------------------------
class SCIF_Unlearner:
    def __init__(self, model, data_gen, device, if_epoch=1000, if_lr=1e-4, reg=0.01):
        self.model = model
    ...
    def _full_loss(self, u_emb, i_emb):
        ...
        return F.binary_cross_entropy_with_logits(logits, labels, reduction="mean") + self.reg * (
            u_emb.norm() + i_emb.norm()
        )
...
# ------------------------------------------------------------
# 5. Forget set evaluation (correct method)
# ------------------------------------------------------------
...
def train_and_unlearn(args):
    ...
    if not os.path.exists(f"{processed_dir}/train_normal.csv"):
        process_yelp_data(raw_inter_path, processed_dir, forget_ratio=args.forget_ratio)
    ...
    combined.to_csv(f"{processed_dir}/{ds}_train.csv", index=False)
    ...
    print(f"\n=== Running SCIF Unlearning (NGCF, Yelp) ===")
    ...
    for k in config["topk"]:
        ...
        print(
            f"  Hit@{k}:    {orig_hit:.6f} -> {unlearn_hit:.6f} (delta: {unlearn_hit - orig_hit:+.6f})"
        )
        print(
    ...
    results_summary = {
        "model": args.model,
        "dataset": ds,
        "scif_params": {"if_epoch": args.if_epoch, "if_lr": args.if_lr},
        "zrf_score": float(zrf),
        "test_original": {
            f"{metric}@{k}": float(res1[f"{metric}@{k}"]) for k in config["topk"] for metric in ["hit", "ndcg", "recall"]
        },
        "test_unlearned": {
            f"{metric}@{k}": float(res2[f"{metric}@{k}"]) for k in config["topk"] for metric in ["hit", "ndcg", "recall"]
        },
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", "-m", default="NGCF")
    parser.add_argument("--if_epoch", type=int, default=1500)
    parser.add_argument("--if_lr", type=float, default=1e-5)
