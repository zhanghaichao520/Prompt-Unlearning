...
def process_netflix_data(original_file, output_dir, forget_ratio=0.01):
    ...
    for u, g in groups:
        ...
        n1 = int(len(g)*0.6); n2 = int(len(g)*0.2)
...
def train_and_unlearn(args):
    ...
    if not os.path.exists(f"{dp}/train_normal.csv"):
        process_netflix_data("scif/netflix.inter", dp, forget_ratio=args.forget_ratio)
    ...
    # 5.2 RecBole CSV
    pd.concat([tn_raw, tr_raw], ignore_index=True).to_csv(f"{dp}/{ds}_train.csv", index=False)
