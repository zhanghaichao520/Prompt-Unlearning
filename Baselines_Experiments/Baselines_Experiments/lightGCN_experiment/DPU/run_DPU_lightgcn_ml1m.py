...
if __name__ == "__main__":
    ...
    backbone_path = "/data/P2F/pretrain_checkpoints/lightgcn_ml1m_full.pth"
    ...
    save_path = os.path.join(project_root, "unlearning_checkpoints", "lightgcn_unlearned_p2f_ml1m.pth")
    if not os.path.exists(os.path.dirname(save_path)):
        os.makedirs(os.path.dirname(save_path))
    torch.save(student.state_dict(), save_path)
    print(f"Saved unlearned model to {save_path}")

    with open(os.path.join(project_root, "lightGCN_experiment/P2F/lightgcn_experiment_results_ml1m.txt"), "w") as f:
