...
# Deletion Layer (Simpler, as per GNNDELETE paper)
...
# NGCF with GNNDELETE
class NGCFGNNDELETE(nn.Module):
    r"""NGCF with GNNDELETE unlearning operator.
    """

    def __init__(self, n_users, n_items, embedding_size=64, n_layers=3, 
                 node_dropout=0.1, message_dropout=0.1, reg_weight=1e-5,
                 prompt_type=None, p_num=5, deletion_lambda=0.5):
        super(NGCFGNNDELETE, self).__init__()
        ...
        self.deletion_lambda = deletion_lambda  # λ for balancing DEC and NI
        self.inference_masks = None # Store masks for inference
        ...
        # Deletion operators for each layer
        self.deletion_ops = torch.nn.ModuleList()
        for idx, output_size in enumerate(self.hidden_size_list[1:]):
            self.deletion_ops.append(DeletionLayer(output_size))
    ...
    def forward(self, norm_adj_matrix, training=True, masks=None, deleted_edges=None):
        ...
        for idx, gnn in enumerate(self.GNNlayers):
            ...
            # Apply deletion operator if masks provided (or stored)
            if current_masks is not None and idx < len(current_masks) and current_masks[idx] is not None:
                all_embeddings = self.deletion_ops[idx](all_embeddings, current_masks[idx])
    ...
    def calculate_loss(self, users, pos_items, neg_items, norm_adj_matrix):
        user_all_embeddings, item_all_embeddings = self.forward(norm_adj_matrix, training=True, masks=None)
    ...
    def calculate_deletion_loss(self, norm_adj_matrix, batch_deleted_edges, masks, subgraph_nodes=None):
        ...
        masks: List of node masks for each layer (for DeletionLayer)
        subgraph_nodes: Tensor of node indices to enforce Local Consistency (NI)
        """
        device = norm_adj_matrix.device
        
        # 1. Forward with deletion operator
        user_all_embeddings_del, item_all_embeddings_del = self.forward(norm_adj_matrix, training=True, masks=masks)
        
        # 2. Forward without deletion (Base model) - No gradients needed for base
        with torch.no_grad():
            user_all_embeddings_base, item_all_embeddings_base = self.forward(norm_adj_matrix, training=True, masks=None)
        ...
        if batch_deleted_edges:
            ...
            u_emb_del = user_all_embeddings_del[u_indices]
            v_emb_del = item_all_embeddings_del[v_indices]
            
            # Score of deleted edges
            score_del = torch.sum(u_emb_del * v_emb_del, dim=1)
            
            # Random scores (Consistency target)
            ...
            rand_u_emb = user_all_embeddings_del[rand_u]
            rand_v_emb = item_all_embeddings_del[rand_v]
            
            # Use Random edge score as target
            score_rand = torch.sum(rand_u_emb * rand_v_emb, dim=1).detach() # Target is fixed
            
            # Minimize difference between deleted edge score and random score
            dec_loss = F.mse_loss(score_del, score_rand)
            
        # --- NI Loss: Neighborhood Influence ---
        # "Representations of local neighborhood should be preserved"
        if subgraph_nodes is not None and len(subgraph_nodes) > 0:
            # We compare the embeddings of the subgraph nodes
            # Concatenate user and item embeddings for easier indexing
            # Note: subgraph_nodes should be in range [0, n_users + n_items)
            
            all_emb_del = torch.cat([user_all_embeddings_del, item_all_embeddings_del], dim=0)
            all_emb_base = torch.cat([user_all_embeddings_base, item_all_embeddings_base], dim=0)
        ...
        if torch.rand(1).item() < 0.01:
             print(f"    [D] DEC: {dec_loss:.6f} | NI: {ni_loss:.6f}")
             
        return total_loss

    def predict(self, users, items, norm_adj_matrix, deleted_edges=None, masks=None):
        user_all_embeddings, item_all_embeddings = self.forward(norm_adj_matrix, training=False, masks=masks, deleted_edges=deleted_edges)
        u_embeddings = user_all_embeddings[users]
        i_embeddings = item_all_embeddings[items]
        scores = torch.sum(u_embeddings * i_embeddings, dim=1)
        return scores
    
    def full_sort_predict(self, users, norm_adj_matrix, deleted_edges=None, masks=None):
        self.restore_user_e = None
        self.restore_item_e = None
        user_all_embeddings, item_all_embeddings = self.forward(norm_adj_matrix, training=False, masks=masks, deleted_edges=deleted_edges)
