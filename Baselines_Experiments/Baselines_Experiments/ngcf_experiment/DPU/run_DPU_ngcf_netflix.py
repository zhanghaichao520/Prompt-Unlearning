...
NGCF_CONFIG = {
    ...
    'batch_size': 3000000, # Full Batch Training (One step per epoch)
    'eval_freq': 5,
    'patience': 10,
}
...
if __name__ == "__main__":
    ...
    dataset = ML100KDataset(absolute_data_path)
    ...
    backbone_path = os.path.join(project_root, "ngcf_experiment", "best_netflix_ngcf_backbone.pth")
    ...
    class RetainDataset(Dataset):
        ...
        def __getitem__(self, idx):
            # ML100KDataset samples are (user, pos_item, neg_item)
            user, pos_item, neg_item = self.samples[idx]
            # We can use the existing negative item
    ...
    UNLEARNING_CONFIG['batch_size'] = 3000000  # Full Batch
    ...
    with open("ngcf_experiment_results_netflix.txt", "w") as f:
