...
NGCF_CONFIG = {
    ...
    'message_dropout': 0.1,
    'lr': 0.001,
    'epochs': 200, # Increased epochs for SGD
    'batch_size': 4096, # Standard Mini-batch for best Recall
    'eval_freq': 5,
    'patience': 10,
}
...
if __name__ == "__main__":
    ...
    # User requested to match RecBole metrics which likely uses 10% test split AND 5-core item filtering
    # RecBole default for ML-1M usually filters rare items (<5 interactions)
    print("Initializing ML100KDataset with test_size=0.1 and min_item_count=5...")
    ...
    # backbone_path relative to project root or current experiment folder
    backbone_path = os.path.join(project_root, "pretrain_checkpoints", "best_ml_1m_ngcf_backbone.pth")
    if os.path.exists(backbone_path):
        print(f"Loading backbone from {backbone_path}...")
        try:
            checkpoint = torch.load(backbone_path, map_location=device)
            if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
                print("Detected RecBole checkpoint structure. Loading 'state_dict'...")
                backbone.load_state_dict(checkpoint['state_dict'])
            else:
                backbone.load_state_dict(checkpoint)
        except RuntimeError as e:
            print(f"Error loading directly: {e}")
            print("Attempting to load with strict=False (ignoring mismatches)...")
            checkpoint = torch.load(backbone_path, map_location=device)
            if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
        ...
        def __getitem__(self, idx):
            ...
            if len(sample) == 3:
                ...
                try:
                    ...
                    # Should not happen if users are consistent
                    pass
            
            # Return scalars to be consistent with ML100KDataset and avoid shape issues
    ...
    UNLEARNING_CONFIG['batch_size'] = 2048    # Keep same batch size
    UNLEARNING_CONFIG['lr'] = 0.005           # Keep same LR
    UNLEARNING_CONFIG['alpha'] = 0.5          # Restore original alpha
    UNLEARNING_CONFIG['patience'] = 10        # Keep increased patience for stability
    ...
    # Save Model
    save_path = os.path.join(project_root, "unlearning_checkpoints", "ngcf_unlearned_p2f_ml1m.pth")
    ...
    with open("ngcf_experiment_results.txt", "w") as f:
