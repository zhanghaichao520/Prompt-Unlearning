...
# NGCF specific config (Same as run_p2f_ngcf_ml1m.py)
NGCF_CONFIG = {
    'embedding_size': 64,
    'n_layers': 3,
    'reg_weight': 1e-5,
...
def train_ngcf(model, dataset, epochs, norm_adj_matrix, save_path=None):
    ...
    train_loader = DataLoader(dataset, batch_size=NGCF_CONFIG['batch_size'], shuffle=True, num_workers=0)
...
if __name__ == "__main__":
    ...
    # Use test_size=0.1 and min_item_count=5 to match standard evaluation metrics
    ...
    save_path = "best_ngcf_retrained_ml1m.pth"
    ...
    with open("ngcf_retrain_ml1m_results.txt", "w") as f:
