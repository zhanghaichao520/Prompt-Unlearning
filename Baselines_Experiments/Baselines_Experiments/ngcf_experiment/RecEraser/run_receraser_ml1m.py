...
def calculate_zrf(rec_eraser, incompetent_teacher, forget_data, device):
    ...
    loader = DataLoader(ListDataset(forget_data), batch_size=1024, shuffle=False)
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="ZRF"):
            ...
            # If teacher is RecEraser, no norm_adj needed. If NGCF, norm_adj needed.
            # Main code creates RecEraser as incompetent teacher
            if hasattr(incompetent_teacher, 'partition'): # Is RecEraser
                 incompetent_logits = incompetent_teacher.predict(users, items)
            else:
                 # Fallback if we pass normalized adj, but here we don't have it easily
                 # Assume RecEraser for consistency with main
                 incompetent_logits = incompetent_teacher.predict(users, items)
...
def evaluate(rec_eraser, dataset, k=[10, 20]):
    # Custom evaluate calling rec_eraser.predict
    # Wrapper not strictly needed if we conform to interface, but RecEraser has predict.
    
    # We need to use test data
    test_user_items = dataset.test_user_items
    train_user_items = dataset.train_user_items # To filter trained items
    
    return evaluate_model(rec_eraser, test_user_items, train_user_items, k, 1024, rec_eraser.device)
...
def main():
    ...
    config = {
        'root': os.path.join(project_root, 'ngcf_experiment', 'RecEraser'),
        'num_shards': 10, # As per paper typical val
        'embedding_size': 64,
        'n_layers': 3,
        'lr': 0.001,
        'shard_epochs': 100, # Reduced for speed in this env, typical 100
        'seed': 2023
    }
    ...
    dataset = ML1MDataset(root=dataset_root, test_size=0.1)
    ...
    if rec_eraser.load_checkpoints():
        ...
        # Partition
        pretrained_path = os.path.join(project_root, 'pretrain_checkpoints', 'ngcf_ml1m_full.pth')
        ...
        start_time = time.time()
        rec_eraser.train_aggregator(epochs=10) # Reduced
    ...
    original_forget = evaluate_model(rec_eraser, forget_user_items, {}, [10, 20], 1024, rec_eraser.device)
    print("Original Forget:")
    print_metrics(original_forget)
    
    
    # Update global dataset before calling unlearn
    # This ensures aggregator uses updated data
    original_len = len(dataset.train_samples)
    
    # Filter train_samples
    ...
    # Update train_user_items for those users
    for u in target_users:
    ...
    # Forget
    print("Evaluating on Forget Set...")
    unlearned_forget = evaluate_model(rec_eraser, forget_user_items, {}, [10, 20], 1024, rec_eraser.device)
    ...
    # Calculate ZRF
    print("Calculating ZRF...")
    # NOTE: In a real scenario, incompetent_teacher should be a truly random model.
    # Here we can instantiate a fresh untrained NGCF model or RecEraser.
    # For simplicity, we create a fresh un-trained RecEraser instance with same config.
    incompetent_teacher = RecEraser(dataset, config, device)
    incompetent_teacher.init_random()
    # No training for teacher
